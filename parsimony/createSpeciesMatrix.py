...
def parseArgs():
	...
	parser.add_argument("-o",help="Output Matrix File",action="store",dest="output", required=False)
...
def removeCodons(speciesList,informativeCodons,pairing,codonToFreq,speciesToNumChars):
	...
	'''
	#print 'Removing uninformative codons and species'
	...
	codonsToDelete = set()
	for species in speciesList:
		if speciesToNumChars[species] / float(len(informativeCodons)) < 0.05:
			#print 'Removing species: ' + species
				...
				if gene in pairing[species]:
					if geneCodon in codonToFreq:
						...
						if codonToFreq[geneCodon] < 4:
							#print 'Removing codon: ' + geneCodon
...
def readInputFiles(args):
	...
	if args.output:
		...
		sys.stdout.write("xread\n")
		sys.stdout.write(str(len(informativeCodons)) + " " + str(len(speciesList)) + "\n")
	...
	for species in speciesList:
		if args.outputNames:
			if args.output:	
				...
				sys.stdout.write("Species_" + str(count) + "\t")
		...
		for item in informativeCodons:
			...
			if gene in pairing[species]:
				...
				if args.output:
					...
					sys.stdout.write(pairing[species][gene][index])
			else:
				if args.output:
					outfile.write('?')
				else:
